...
impl<'a> Tokenizer<'a> {
    ...
    ) -> Result<Result<Option<(Span, Token<'a>)>, Error>, (Span, toml_lexer::StrLitSubtoken)> {
    ...
    ) -> Result<Option<(Span, Token<'a>)>, Error> {
        let mut unescaped = MaybeString::new(self.input(), leading_quotes_span.end);
    ...
    pub(crate) fn peek(&self) -> Result<Option<(Span, Token<'a>)>, Error> {
        // FIXME: when the next token is a string literal we may
        // eagerly allocate a string to unescape it
        self.clone().next()
    }
...
#[derive(Debug)]
enum MaybeString<'a> {
    NotEscaped(&'a str, str::CharIndices<'a>, usize),
    Escaped(String),
}

// TODO: test
impl<'a> MaybeString<'a> {
    fn new(source: &str, begin: usize) -> MaybeString<'_> {
        MaybeString::NotEscaped(source, source[begin..].char_indices(), begin)
    }
    ...
    fn skip(&mut self, span: Span) {
        match self {
            ...
            MaybeString::NotEscaped(source, char_indices, begin) if span.start == *begin => {
                *begin = span.end;
            ...
            MaybeString::Escaped(_) => {}
        }
    }

    fn append(&mut self, ch: char) {
        match self {
            MaybeString::NotEscaped(source, char_indices, begin) => {
                ...
                if ch == contents_char {
                    ...
                    *self = MaybeString::Escaped(format!("{}{}", &source[*begin..*begin + i], ch));
                }
            }
            MaybeString::Escaped(it) => it.push(ch),
        }
    }

    fn into_cow(self) -> Cow<'a, str> {
        match self {
            MaybeString::NotEscaped(source, mut char_indices, begin) => {
                match char_indices.next() {
                    Some((end, _)) => Cow::Borrowed(&source[begin..begin + end]),
                    None => Cow::Borrowed(&source[begin..]),
                }
            }
            MaybeString::Escaped(it) => Cow::Owned(it),
        }
    }
}
