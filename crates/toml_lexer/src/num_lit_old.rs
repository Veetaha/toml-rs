...
enum NumLitToken {
    Signful(NumSign, Option<NumLitTokenKind>),
    Signless(NumLitTokenKind),
}
...
enum NumLitTokenKind {
    Float(FloatVal),
    Int { kind: IntKind, val: Option<IntVal> },
}
...
enum IntKind {
    ExplicitRadix(IntRadix),
    Decimal { n_leading_zeros: u32 },
}
enum IntVal {
    Val(i64),
    Overflow,
...
fn tokenize_num_lit(
    input: &str,
    report_bad_underscores: impl FnMut(Span),
) -> Option<(Span, NumLitToken)> {
    NumLexer {
        cursor: Cursor::new(input),
        report_bad_underscores,
    }
    .tokenize_num_lit()
}
...
struct NumLexer<'s, R> {
    cursor: Cursor<'s>,
    report_bad_underscores: R,
}

impl<'s, R: FnMut(Span)> NumLexer<'s, R> {
    fn tokenize_num_lit(&mut self) -> Option<(Span, NumLitToken)> {
